...
INTERN bool calc_zdst_lists( void )
{
	...
	for ( cmp = 0; cmp < cmpc; cmp++ )
	{
		...
		for ( bpos = 1; bpos < 64; bpos++ ) {
			...
			if ( b_x == 0 ) {
				for ( dpos = 0; dpos < cmpnfo[cmp].bc; dpos++ )
					if ( colldata[cmp][bpos][dpos] != 0 ) pjg::zdstylow[cmp][dpos]++;
			}
			else if ( b_y == 0 ) {
				for ( dpos = 0; dpos < cmpnfo[cmp].bc; dpos++ )
					if ( colldata[cmp][bpos][dpos] != 0 ) pjg::zdstxlow[cmp][dpos]++;
			}
			else {
				for ( dpos = 0; dpos < cmpnfo[cmp].bc; dpos++ )
					if ( colldata[cmp][bpos][dpos] != 0 ) pjg::zdstdata[cmp][dpos]++;
			}
		}
	}
...
bool pjg::encode::encode()
{
	...
	pjg::encode::bit(encoder, padbit);
	// also encode one bit to signal false/correct use of RST markers
	pjg::encode::bit(encoder, (rst_err == NULL) ? 0 : 1);
	...
	pjg::encode::bit(encoder, (grbs > 0) ? 1 : 0);
...
void pjg::encode::zdst_low(const std::unique_ptr<aricoder>& enc, int cmp)
{
	...
	zdstls_x = pjg::zdstxlow[ cmp ];
	zdstls_y = pjg::zdstylow[ cmp ];
	ctx_eobx = pjg::eobxhigh[ cmp ];
	ctx_eoby = pjg::eobyhigh[ cmp ];
	ctx_zdst = pjg::zdstdata[ cmp ];
	bc = cmpnfo[cmp].bc;
...
void pjg::encode::ac_high(const std::unique_ptr<aricoder>& enc, int cmp)
{
	...
	sgn_nbh = sgn_store.data() - 1;
	sgn_nbv = sgn_store.data() - w;
	
	// locally store pointer to eob x / eob y
	eob_x = pjg::eobxhigh[ cmp ];
	eob_y = pjg::eobyhigh[ cmp ];
...
void pjg::decode::ac_high(const std::unique_ptr<aricoder>& dec, int cmp)
{
	...
	for ( dpos = 0; dpos < bc; dpos++ )
		zdstls[ dpos ] = pjg::zdstdata[ cmp ][ dpos ];
...
/* -----------------------------------------------
	...
	----------------------------------------------- */
void pjg::decode::unoptimize_header()
{
	unsigned char  type = 0x00; // type of current marker segment
	unsigned int   len  = 0; // length of current marker segment
	unsigned int   hpos = 0; // position in header
